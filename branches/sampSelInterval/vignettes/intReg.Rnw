...
= & \int_{- \infty}^{x_1} \int_{- \infty}^{x_2}
   ...
   \right)\right) \cdot \sqrt{1-\rho^2}}{ 1 - \rho^2 }  \\
  & \qquad - \dfrac{ 
...
= & \int_{- \infty}^{x_1} \int_{- \infty}^{x_2}
   \frac{1}{2\pi} \Bigg(
   \dfrac{
   -\dfrac{1}{2}\dfrac{\partial}{\partial \rho} \left( \dfrac{a_1^2 - 2 
   \rho a_1 a_2 + a_2^2}{1-\rho^2} \right) \cdot       
    \exp \left( -\dfrac{a_1^2 - 2 \rho a_1 a_2 + a_2^2}{2(1-\rho^2)} \right) 
    \cdot \sqrt{1-\rho^2}} { 1 - \rho^2 } \\
  & \qquad - \dfrac {
    (-\dfrac{2\rho}{2 \sqrt{1-\rho^2}}) \cdot \exp \left( -\dfrac{a_1^2 - 
...
= & \int_{- \infty}^{x_1} \int_{- \infty}^{x_2}
   \frac{1}{2\pi} \Bigg(
   \dfrac{
   -\dfrac{1}{2}\dfrac{-2 a_1 a_2 \cdot (1-\rho^2) - (-2\rho) \cdot (a_1^2 - 
   2\rho a_1 a_2 + a_2^2) \cdot       
   \exp \left( -\dfrac{a_1^2 - 2 \rho a_1 a_2 + a_2^2}{2(1-\rho^2)} \right) 
   \cdot \sqrt{1-\rho^2}}{(1-\rho^2)^2}}
   { 1 - \rho^2 }  \\
  & \qquad -\dfrac{
...
\subsection{Gradients with respect to the parameters in the self-selection 
decision ($\vec{\beta}^S$)}
...
$\vec{\beta}^S$ as part of the loglikelihood function. We use $z$ as variable 
for the lower boundary, with $\lim{z}_{z->-\infty}=-\infty$ :

\begin{align}
& \frac{\partial \Phi_2 \left( \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}
    {\sigma_2}, {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right)}{\partial \beta^S} 
   = \frac{\partial}{\partial \beta^S} \left[ \int_{z}^{\frac{\alpha_{m} - 
    {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2}} \int_{z}^{{\vec{\beta}^S}' 
    \vec{x}_i^S} \phi_2 \left( a_1, a_2, - \rho \right) 
    \; d a_2 \; d a_1 \right]
\\
& = \int_{z}^{\frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2}} \Bigg[
    \frac{\partial}{\partial \beta^S} \left( {\vec{\beta}^S}' \vec{x}_i^S 
    \right) \cdot \phi_2(a_1, {\vec{\beta}^S}' \vec{x}_i^S, , -\rho) -
    \frac{\partial}{\partial \beta^S} (z) \cdot \phi_2(a_1, z, -\rho)
    \Bigg] \; d a_1
\\
...
& = ( 1 - y_i^S )
   ...
   \right] \Bigg) \nonumber \\
& = ( 1 - y_i^S )
    \frac{\phi \left( - {\vec{\beta}^S}' \vec{x}_i^S \right) \cdot \vec{x}_i^S}
    {\Phi \left( - {\vec{\beta}^S}' \vec{x}_i^S \right)}  \\
    & \quad + \sum_{m=1}^M y_i^S ( y_i^O = m )
      \frac{\frac{\partial}{\partial \beta^S} \Bigg( \Phi_2 \left(
         \frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
         {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) \Bigg)
         - \frac{\partial}{\partial \beta^S} \Bigg( \Phi_2 \left(
         \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
         {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) \Bigg)}
         {\Phi_2 \left(
         \frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
...
& = ( 1 - y_i^S )
    ...
    {\Phi \left( - {\vec{\beta}^S}' \vec{x}_i^S \right)} 
    \\
   & \quad + \sum_{m=1}^M y_i^S ( y_i^O = m ) \cdot \Bigg(
      \frac{ \Phi \left( \frac{\frac{\alpha_{m+1} - {\vec{\beta}^O}' 
      \vec{x}_i^O}{\sigma_2} 
      - \rho {\vec{\beta}^S}' \vec{x}_i^S)}{\sqrt{1 - \rho^2}} \right)
      \phi( {\vec{\beta}^S}' \vec{x}_i^S ) \cdot \vec{x}_i^S}
        {\Phi_2 \left(
        ...
        \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
        {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) } \nonumber \\
        & \quad - \frac{\Phi \left( \frac{\frac{\alpha_{m} - {\vec{\beta}^O}' 
         \vec{x}_i^O}{\sigma_2} 
      - \rho {\vec{\beta}^S}' \vec{x}_i^S)}{\sqrt{1 - \rho^2}} \right)
        \phi( {\vec{\beta}^S}' \vec{x}_i^S ) \cdot \vec{x}_i^S} 
            {{\Phi_2 \left(\frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}
            {\sigma_2}, {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) - \Phi_2 
            \left(\frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
            {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) }} \Bigg) \nonumber
\end{align}
...
\subsection{Gradients with respect to the parameters in the outcome decision 
($\vec{\beta}^O$)}

%\frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2}
%{\vec{\beta}^S}' \vec{x}_i^S
Analogous to $\vec{\beta}^S$ we derive the gradient of $\vec{\beta}^O$:
\begin{align}
& \frac{\partial \Phi_2 \left( \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}
    {\sigma_2}, {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right)}{\partial \beta^O} 
    = \frac{\partial}{\partial \beta^O} \left[ \int_{z}^{\frac{\alpha_{m} - 
    {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2}} \int_{z}^{{\vec{\beta}^S}'
    \vec{x}_i^S} \phi_2 \left( a_1, a_2, - \rho \right) 
    \; d a_2 \; d a_1 \right]
\\
& = \frac{\partial}{\partial \beta^O} \left[ \int_{z}^{{\vec{\beta}^S}' 
    \vec{x}_i^S} \int_{z}^{\frac{\alpha_{m} - {\vec{\beta}^O}' 
    \vec{x}_i^O}{\sigma_2}} \phi_2 \left( a_1, a_2, - \rho \right) 
    \; d a_1 \; d a_2 \right]
\\
& = \int_{z}^{{\vec{\beta}^S}' \vec{x}_i^S} \Bigg[
      \frac{\partial}{\partial \beta^O} \left( \frac{\alpha_{m} - 
      {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2} \right)
      \cdot \phi_2(\frac{\alpha_{m} - {\vec{\beta}^O}' 
      \vec{x}_i^O}{\sigma_2}, a_2, -\rho) -
      \frac{\partial}{\partial \beta^O} (z) \cdot \phi_2(z, a_2, -\rho)
      \Bigg] \; d a_2
\\
& = \int_{z}^{{\vec{\beta}^S}' \vec{x}_i^S} \left[
      \phi_2(\frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}
      {\sigma_2}, a_2, -\rho) \right] \; d a_2 \cdot (-\frac{\vec{x}_i^O}
      {\sigma_2})
\\
& = \Phi \left( \frac{{\vec{\beta}^S}' \vec{x}_i^S 
      - \rho \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2}}
      {\sqrt{1 - \rho^2}} \right) \phi( \frac{\alpha_{m} - {\vec{\beta}^O}' 
      \vec{x}_i^O}{\sigma_2} ) \cdot (-\frac{\vec{x}_i^O}{\sigma_2})
\end{align}
...
& = \sum_{m=1}^M y_i^S ( y_i^O = m )
      \frac{\frac{\partial}{\partial \beta^O} \Bigg( \Phi_2 \left(
         \frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
         {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) \Bigg)
         - \frac{\partial}{\partial \beta^O} \Bigg( \Phi_2 \left(
         \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
         {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) \Bigg)}
         {\Phi_2 \left(
         \frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
         ...
         \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
         {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) } \\
& = \sum_{m=1}^M y_i^S ( y_i^O = m ) \cdot \Bigg(
        \frac{ \Phi \left( \frac{{\vec{\beta}^S}' \vec{x}_i^S 
        - \rho \frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2}}
        {\sqrt{1 - \rho^2}} \right) \phi( \frac{\alpha_{m+1} - 
        {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2} ) \cdot (-\frac{\vec{x}_i^O}
        {\sigma_2})} 
            {\Phi_2 \left(
            \frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
            {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) - \Phi_2 \left(
...
to the PDF, we can also derive the gradient of the correlation parameter 
($\rho$):
...
normalisation ($\sigma_2$)}

\begin{align}
& \frac{\partial \Phi_2 \left( \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}
    {\sigma_2}, {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right)}{\partial \sigma_2} 
         = \frac{\partial}{\partial \sigma_2} \left[ \int_{z}^{{\vec{\beta}^S}' 
         \vec{x}_i^S} \int_{z}^{\frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}
         {\sigma_2}} \phi_2 \left( a_1, a_2, - \rho \right) 
         \; d a_1 \; d a_2 \right]
\\
& = \int_{z}^{{\vec{\beta}^S}' \vec{x}_i^S} \Bigg[\frac{\partial}{\partial 
    \sigma_2} \left( \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2} 
    \right) \cdot \phi_2(\frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}
    {\sigma_2}, a_2, -\rho) - \frac{\partial}{\partial \sigma_2} (z) \cdot 
    \phi_2(z, a_2, -\rho)
    \Bigg] \; d a_2
\\
& = \int_{z}^{{\vec{\beta}^S}' \vec{x}_i^S} \left[
      ...
      \right] \; d a_2 \cdot \frac{{\vec{\beta}^O}' \vec{x}_i^O - \alpha_{m}}
      {\sigma_2^2}
\\
& = \Phi \left( \frac{{\vec{\beta}^S}' \vec{x}_i^S 
      - \rho \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2}}
      {\sqrt{1 - \rho^2}} \right) \phi( \frac{\alpha_{m} - {\vec{\beta}^O}' 
      \vec{x}_i^O}{\sigma_2} ) \cdot \frac{{\vec{\beta}^O}' \vec{x}_i^O - 
      \alpha_{m}}{\sigma_2^2}
\end{align}

\begin{align}
& \frac{\partial \ell_i}{\partial \sigma_2} = \frac{\partial}{\partial \sigma_2} 
    \Bigg( ( 1 - y_i^S ) \ln \left[ \Phi \left( - {\vec{\beta}^S}' 
    \vec{x}_i^S \right) \right] \\
   & \quad + \sum_{m=1}^M y_i^S ( y_i^O = m ) \ln \left[
      \Phi_2 \left(
      ...
      & \qquad \left. - \Phi_2 \left(
         ...
         {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) 
   \right] \Bigg) \nonumber \\
& = \sum_{m=1}^M y_i^S ( y_i^O = m ) \frac{\partial}{\partial \sigma_2} 
        \Bigg( \ln \left[\Phi_2 \left(
        \frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
        {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) \right. \\
      & \qquad \left. - \Phi_2 \left(
   ...
   \right] \Bigg) \nonumber \\
& = \sum_{m=1}^M y_i^S ( y_i^O = m )
      \frac{\frac{\partial}{\partial \sigma_2} \Bigg( \Phi_2 \left(
         \frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
         {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) \Bigg)
         - \frac{\partial}{\partial \sigma_2} \Bigg( \Phi_2 \left(
         \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
         {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) \Bigg)}
         {\Phi_2 \left(
         \frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
...
& = \sum_{m=1}^M y_i^S ( y_i^O = m ) \cdot \Bigg(
      \frac{ \Phi \left( \frac{{\vec{\beta}^S}' \vec{x}_i^S 
      - \rho \frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2}}
      {\sqrt{1 - \rho^2}} \right) \phi( \frac{\alpha_{m+1} - {\vec{\beta}^O}' 
      \vec{x}_i^O}{\sigma_2} ) \cdot \frac{{\vec{\beta}^O}' 
      \vec{x}_i^O - \alpha_{m+1}}{\sigma_2^2}}
         {\Phi_2 \left(
         \frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
         {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) - \Phi_2 \left(
         \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
    ...
    & - \frac{\Phi \left( \frac{{\vec{\beta}^S}' \vec{x}_i^S 
      ...
      {\sqrt{1 - \rho^2}} \right) \phi( \frac{\alpha_{m} - {\vec{\beta}^O}' 
      \vec{x}_i^O}{\sigma_2} ) \cdot \frac{{\vec{\beta}^O}' 
      \vec{x}_i^O - \alpha_{m}}{\sigma_2^2}} {{\Phi_2 \left(
         \frac{\alpha_{m+1} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
         {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) - \Phi_2 \left(
         \frac{\alpha_{m} - {\vec{\beta}^O}' \vec{x}_i^O}{\sigma_2},
         {\vec{\beta}^S}' \vec{x}_i^S, - \rho \right) }} \Bigg) \nonumber
