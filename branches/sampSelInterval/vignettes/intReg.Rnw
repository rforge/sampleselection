...
we estimate $\log( \sigma^2 )$ instead of $\sigma$ or $\sigma^2$
so that the derived parameters
$\sigma = \sqrt{ \exp( \log( \sigma^2 ) ) }$
and $\sigma^2 = \exp( \log( \sigma^2 ) )$
...
\begin{align}
\frac{\partial \sqrt{ \exp( \log( \sigma^2 ) ) } }{
   \partial \log( \sigma^2 )}
= \frac{\partial \sigma }{\partial \log( \sigma^2 )}
= \frac{\sigma }{2}\\[3ex]
\frac{\partial \exp( \log( \sigma^2 ) )}{\partial \log( \sigma^2 )}
= \frac{\partial \sigma^2}{\partial \log( \sigma^2 )}
= \sigma^2
\end{align}
...
\begin{align}
\frac{\partial \ell_i}{\partial \log(\sigma^2)}
