...
\subsection{Gradients of the CDF of the bivariate normal distribution}
...
\subsubsection{Gradients with respect to the limits ($x_1$ and $x_2$)}
...
\subsubsection{Gradients with respect to the coefficient of correlation ($\rho$)}
...
& =   \int_{- \infty}^{x_2} \int_{- \infty}^{x_1}
         ...
         \\
         & exp \left (-\frac{1}{2(1-\rho^2)} \left (\frac{(a_1-\mu_1)^2}{\sigma_1^2}
         \cdot -2\rho \frac{(a_1 - \mu_1)(a_2-\mu_2)}{\sigma_1 \sigma_2}
         + \frac{(a_2 - \mu_2)^2}{\sigma_2^2} \right ) \right )
      \; d a_1 \; d a_2
\\ \nonumber
& =   \int_{- \infty}^{x_2} \int_{- \infty}^{x_1}
         \frac{\rho}{2 \pi \sigma_1 \sigma_2 (1-\rho^2)^{3/2}} \cdot
         \\
         & exp \left (-\frac{1}{2(1-\rho^2)} \left (\frac{(a_1-\mu_1)^2}{\sigma_1^2}
         \cdot -2\rho \frac{(a_1 - \mu_1)(a_2-\mu_2)}{\sigma_1 \sigma_2}
         + \frac{(a_2 - \mu_2)^2}{\sigma_2^2} \right ) \right )
...
funrho <- function( p ) {
   prob <- pmvnorm( upper = c( x1, x2 ), sigma = matrix( c( 1, p, p, 1 ), nrow = 2 ) )
   return( prob )
}
grad <- c( numericGradient( funrho, rho ) )
print( grad )

# Analytical gradient for rho
