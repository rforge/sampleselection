...
round( vcov( estResult ), 3 )
round( vcov( estResult, logSigma = FALSE ), 3 )
...
meat( estResult )
round( bread( estResult ), 3 )
round( sandwich( estResult ), 3 )
...
round( vcov( estResultAdd ), 3 )
round( vcov( estResultAdd, logSigma = FALSE ), 3 )
logLik( estResultAdd )
nobs( estResultAdd )
...
round( vcov( estResultNeg ), 3 )
round( vcov( estResultNeg, logSigma = FALSE ), 3 )
logLik( estResultNeg )
nobs( estResultNeg )
...
round( vcov( estResultAddNeg ), 3 )
round( vcov( estResultAddNeg, logSigma = FALSE ), 3 )
...
round( vcov( estResultBoth ), 3 )
round( vcov( estResultBoth, logSigma = FALSE ), 3 )
...
meat( estResultBoth )
round( bread( estResultBoth ), 3 )
round( sandwich( estResultBoth ), 3 )
...
meat( estResultEmpty )
round( bread( estResultEmpty ), 3 )
round( sandwich( estResultEmpty ), 3 )
...
round( c( logLikBhhh ), 3 )
round( attr( logLikBhhh, "gradient" ), 3 )
all.equal( sum( logLikBhhh ), c( logLik( estResultBhhh ) ) )
logLikStart <- censReg( affairsFormula, data = Affairs,
   start = c( 8.17, -0.18, 0.55, -1.69, 0.33, -2.3, 2.13 ),
   logLikOnly = TRUE )
round( c( logLikStart ), 3 )
round( attr( logLikStart, "gradient" ), 3 )
