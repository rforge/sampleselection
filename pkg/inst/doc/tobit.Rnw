...
y_i &=
   ...
   \end{cases}
\end{align}
Here the subscript $i$ indicates the observation,
...
\log L  = \;&
   \sum_{y_i = a}
      \log \pnorm \left( \frac{ a - x_i ' \beta }{ \sigma } \right)\\
   & + \sum_{a < y_i < b}
      \left(
      ...
      \right) \nonumber \\
   & + \sum_{y_i = b}
      \log \pnorm \left( \frac{ x_i ' \beta - b }{ \sigma } \right)
      \nonumber
\end{align}
This log-likelihood function can be maximised using standard non-linear
