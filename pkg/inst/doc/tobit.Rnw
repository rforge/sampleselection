...
<<>>=
tobitResult <- tobit( affairs ~ age + yearsmarried + religiousness +
...
can be estimated by
<<>>=
tobitResultMinus <- tobit( I( - affairs ) ~ age + yearsmarried + religiousness +
...
<<>>=
cbind( coef( tobitResult ), coef( tobitResultMinus ) )
@
...
\begin{align}
y_{it}^* &= x_i ' \beta + \varepsilon_{it} =  x_i ' \beta + \mu_{i} + \eta_{it} \\
y_{it} &=
   \begin{cases}
...
and $\eta_{it}$ is the remaining disturbance.
...
that the remaining disturbance $\eta$ follows a normal distribution
with mean $0$ and variance $\sigma_\eta^2$, and
that $\mu$ and $\eta$ are independent,
the likelihood contributiom of a single individual $i$ is
\begin{align}
L_i  = \int_{-\infty}^\infty
   \Bigg\{ \prod_{t=1}^{T_i} \;&
      \left[
         \pnorm \left( \frac{ a - x_i ' \beta - \mu_i }{ \sigma_\eta } \right)
      \right]^{I_i^a}
      \left[
         \pnorm \left( \frac{ x_i ' \beta + \mu_i - b }{ \sigma_\eta } \right)
      \right]^{I_i^b}\\
      & \left[
         \frac{1}{\sigma_\eta}
         \dnorm \left( \frac{ y_i - x_i ' \beta - \mu_i }{ \sigma_\eta } \right)
      \right]^{\left( 1 - I_i^a - I_i^b \right)}
...
usind the method of Maximum Simulated Likelihood (MSL),
