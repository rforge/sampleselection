...
policy-making.  This is a welcome trend but it also calls for estimation of
causal parameters.  Causal parameters can be directly interpreted as ``impact'': how much
does the variable of interest increase if we change a policy
parameter?  Unfortunately, such effects are hard to estimate based on
...
variables.  For instance, workers who sign up for a training program
may be more motivated or faster learners than those who do not enter
the program.  And if their post-program outcome-of-interest differs, this just can
reflect the obvious: different people behave in a different way.
Unfortunately, the gold standard for measuring
causal effects, randomized experiments, are often not too expensive,
or completely unfeasible.  

An econometric solution is offered by \citet{heckman1976}.  The paper suggests to
...
shows that these assumptions are often unrealistic, the model remain
popular in many applications due to it's simplicity and few additional
demands on data.  Below, we first describe the model and later
illustrate it's usage with \texttt{sampleSelection} package.
...
$y^{s*}$ ($s$ stands for ``selection'') and ``outcome'' $y^{o*}$:
\begin{equation}
  \begin{split}
...
y^{o*}$.  The
parameter of interest is $\beta_{2}$.  Note that this specification
...
The model can also be easily estimated by full Maximum Likelihood.
...
model log-likelihood in the model parameters and observed data as
\begin{multline}
  \label{eq:log_likelihood}
  \loglik =
  -\frac{N}{2} \log 2\pi 
  - N \log \sigma 
  - \frac{N}{2} \left( \frac{y_{i}^{o} - 
\beta_{0} - \vec{\beta}_{1}' \vec{x}_{i}^{o} 
- \beta_{2} y_{i}^{s}}{\sigma} \right)^{2}
  ...
  \log \Phi \left( 
    \frac{-z_{i} - \frac{\displaystyle \rho}{\sigma} 
      \left( y_{i}^{o} - 
        \beta_{0} - \vec{\beta}_{1}' \vec{x}_{i}^{o} 
...
Let us analyze a real dataset (treatment data from library
