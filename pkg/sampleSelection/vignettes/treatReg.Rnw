...
\title{Treatment Effects with Normal Disturbances with
...
policy-making.  This is a welcome trend but it is demanding in terms
of providing corresponding research-based evidence.  Typically the
question-of-interest in policy problems are causal effects: how much
will the variable of interest increase or decrease if we change a
policy parameter?  Unfortunately, such effects are hard to estimate
based on commonly available data.  The reasons are related to
different sample selection effects, the
fact that these are typically different people or economies that face different
...
enter the program.  And if their post-program outcome-of-interest
...
causal effects, randomized experiments, are sometimes too expensive and
sometimes completely unfeasible.
...
demands on data.  Below, we describe the model and later
...
$y^{s*}$ ($s$ stands for ``selection'' and asterisk $^{*}$ means this
variables is not directly observed) and ``outcome'' $y^{o*}$:
\begin{equation}
  \begin{split}
  ...
  \label{eq:the_model}
\end{equation}
where $u$ and $v$ are normally distributed disturbance terms:
\begin{equation}
  \begin{pmatrix}
    u \\ v
  \end{pmatrix}
...
(exclusion restrictions) but it is not necessary.  If participation is
decided based on outcome (for instance, when individuals select the
training only if they expect it to pay off), one may also require that $\vec{x}^{s}$ includes
all the components of $\vec{x}^{o}$.

Instead of the latent participation tendency we observe actual
participation that occurs if $y^{s*} > 0$, and outcome $y^{o} =
y^{o*}$.  The
parameter of interest is $\beta_{2}$ that measures how much changes
$y^{o}$ given someone chooses participation instead of non-participation.
...
The model is straightforward to be estimated by Maximum Likelihood.
...
estimates treatment effect model.

First we provide a usage example based on random data.  We use highly
correlated error terms ($\rho=0.8$), and set all the coefficients equal
to unity:
<<generate_data>>=
N <- 2000
sigma <- 1
...
This is not surprising as have
specified the model correctly.  We also rather precisely (and with a
very high $t$-value) recover the
error term correlation 0.8.


\subsection{Labor Market Training Data}
\label{sec:training_data}

However, the real life is almost never that easy.  The example above
involves 
...
Let us analyze real data instead, treatment data from library
...
We also see that while the error terms in the first case above were slightly
...
\code{sampleSelection} offers an easy interface to estimate the
treatment effect models with spherical disturbances.  Despite the
often disputed assumptions, these models remain popular in applied
research. 
