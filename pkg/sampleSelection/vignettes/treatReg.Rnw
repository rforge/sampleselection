...
best'' outcomes anyway.  Note also that the estimates for the
...
print(summary(tm))
@ 
The estimates are divided into three blocks: the first one describes
the selection equation, the next one the outcome, and the last block
describes the error terms.
In this case all the estimates are close to the correct values.
This is not surprising because in this case we were able to use the
correctly specified model.  We also rather precisely recover the
...
are more likely to
be trained that ``others''.  To a big surprise, the trainings seems to
...
the participants seem to be draw from the weakest end of the
observable skill distribution.  We would accordingly expect they the
correlation to be strongly negative.  Seems like this data is too
coarse to correctly estimate the bias.
