...
\author{Arne Henningsen, G\'eraldine Henningsen, Sebastian Petersen}
...
the model with more explanatory variables (\code{SmaokeRes2})
has significantly higher explanatory power
than the model with fewer explanatory variables~(\code{SmakeRes1}).
