...
\title{Sample Selection Models in \proglang{R}: Package \pkg{sampleSelection2}}
\Plaintitle{Sample Selection Models in R: Package sampleSelection2}

\Abstract{ 
  This introduction to the \proglang{R} package \pkg{sampleSelection2} 
  ...
  the implementation of these models in the package \pkg{sampleSelection2} and
...
package \pkg{sampleSelection2} (version~0.6-0), which is available on
the Comprehensive \proglang{R} Archive Network 
at \url{http://CRAN.R-project.org/package=sampleSelection2}.  The package implements two types
...
of the model in \pkg{sampleSelection2} and its possible generalisations.
...
\section[Implementation]{Implementation in \pkg{sampleSelection2}}
...
\pkg{sampleSelection2} is the command \code{selection}.  It requires a formula for
...
The \pkg{sampleSelection2} package provides several methods for objects
of this class:
...
data set used in this example is included in \pkg{sampleSelection2}; it is
...
\pkg{sampleSelection2}, where it is called \code{RandHIE}.
...
implementation in the package \pkg{sampleSelection2} for the programming
...
switching regression (Tobit-5) models in \pkg{sampleSelection2}, and possible
